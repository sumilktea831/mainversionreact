...
//設定cors跨網域資料來源白名單
var whitelist = ['http://localhost:3000', 'http://192.168.27.51:3000', undefined]
//設定白名單(除此名單以外皆不允許)
//沒有來源網域(即自身網域內的頁面)會被認為是undefined，故要加上此設定才可內連
var corsOptions = {
  credentials: true,
  origin: function (origin, callback) {//origin:來源網域
    console.log('origin:' + origin);
    if (whitelist.indexOf(origin) !== -1) {//判斷來源是否在白名單陣列內
      callback(null, true)
    } else {
...
router.post('/sendmail', function (req, res) {
  ...
  var mailOptions = {
    ...
    <h2>親愛的<span style="color:#ffa510"> ${req.body.nickname} </span>您好：</h2>
    <h2>Movieee平台誠摯的歡迎您的加入!</h2>
    <h3>趕快點擊以下連結進行登入，開始創造屬於你自己的收藏清單吧!</h3>
    <br>
    <a href="http://localhost:3000/LoginSign/?id=${req.body.id}" style="text-decoration: none">>> 點擊我，直接進入Movieee登入頁面吧~</a>
</div>`
  };
  //發送郵件
